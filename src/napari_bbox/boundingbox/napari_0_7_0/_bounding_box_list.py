...
# [NOTE] set of new functions to accomodate new logic:
...
class MeshArrayDict(TypedDict):
    """Mesh array dict used for adding multiple bounding boxes at once.

    This type is helper for functions used when adding multiple bounding boxes at once.
    To ensure names and types of arrays are consistent, we use a TypedDict.

    Fields
    ------
    z_index : ZOrderArray
        Information about z-ndex of bounding boxes.
    vertices : CoordinateArray
        array of bounding box vertices
    ...
    vertices_index : IndexArray
        array with information which range of vertices belongs to which bounding box.
    mesh_triangles_index : IndexArray
        array with information which range of triangles belongs to which bounding box.
    mesh_vertices_index : IndexArray
        array with information which range of mesh vertices belongs to which bounding box.
    """

    z_index: ZOrderArray  # information about z-order of bounding boxes
    vertices: CoordinateArray  # vertices of bounding boxes
    ...
    vertices_index: IndexArray  # offset of vertices for each bounding box
    mesh_triangles_index: (
        IndexArray  # offset of triangles and mesh vertices for each bounding box
    )
    mesh_vertices_index: IndexArray  # offset of mesh vertices for each bounding box


_SizeInformation = tuple[int, int, int, int, int]


def _ensure_color_arrays(bounding_boxes, face_colors=None, edge_colors=None):
    """Return as many face and edge colors as there are bounding boxes in the input.

    Parameters
    ----------
    bounding_boxes : iterable of BoundingBox
        Each bounding box must be a subclass of BoundingBox
    ...
    if face_colors is None:
        face_colors = np.tile(
            np.array([1, 1, 1, 1], dtype=np.float32), (len(bounding_boxes), 1)
        )
    else:
...
def _calculate_array_sizes(bounding_boxes: Iterable[BoundingBox]) -> _SizeInformation:
    ...
    for bounding_box in bounding_boxes:
        n_vertices += len(bounding_box.data_displayed)
        n_indices += len(bounding_box.data)
        n_mesh_vertices += len(bounding_box._face_vertices) + len(
            bounding_box._edge_vertices
        )
        n_face_tri += len(bounding_box._face_triangles)
        n_edge_tri += len(bounding_box._edge_triangles)

    return n_vertices, n_indices, n_mesh_vertices, n_face_tri, n_edge_tri


def _preallocate_arrays(
    bounding_boxes: Sequence[BoundingBox], sizes: _SizeInformation
) -> MeshArrayDict:
    """Preallocate arrays for storing bounding box data.

    Parameters
    ----------
    bounding_boxes : Sequence of bounding boxes
        Each bounding box must be a subclass of BoundingBox
    sizes : tuple
    ...
    # Determine the displayed dimensionality from the first bounding box
    # All bounding boxes in a batch will have the same dimensionality
    dim: Literal[2, 3] = bounding_boxes[0]._face_vertices.shape[1]

    z_index = np.empty(n_bounding_boxes, dtype=np.int32)

    vertices_index = np.empty(n_bounding_boxes, dtype=IndexDtype)
    mesh_triangles_index = np.empty(n_bounding_boxes, dtype=IndexDtype)
    mesh_vertices_index = np.empty(n_bounding_boxes, dtype=IndexDtype)
...
def _fill_arrays( # <------------------------[TRYING TO GET THIS AND AT IMPLEMENTATION BELOW]
    ...
    """Fill pre-allocated arrays with bounding box data.

    Parameters
    ----------
    start_mesh_index : int
        The number of mesh vertices in the bounding box list before adding these
        bounding boxes.
    start_triangle_index : int
        The number of mesh triangles in the bounding box list before adding these
    bounding boxes : iterable of BoundingBox
        Each bounding box must be a subclass of BoundingBox
    face_colors : np.ndarray
        Array of face colors
    edge_colors : np.ndarray
    ...
    for i, (bounding_box, face_color, edge_color) in enumerate(
        ...
        # Store z_index ("number of bounding boxes" space)
        z_index[i] = bounding_box.z_index
        ...
        mesh_triangles_index[i] = (
            start_triangle_index + triangles_offset + bounding_box.triangles_count
        )
        mesh_vertices_index[i] = (
            start_mesh_index + mesh_vertices_offset + bounding_box.vertices_count
        )
        ...
        face_vertices = bounding_box._face_vertices
        ...
        face_triangles = (
            bounding_box._face_triangles + start_mesh_index + mesh_vertices_offset
        )
        ...
        edge_vertices = bounding_box._edge_vertices
        edge_offsets = bounding_box._edge_offsets
        n_edge_vertices = len(edge_vertices)

        # Store edge vertices
        curr_vertices = edge_vertices + bounding_box.edge_width * edge_offsets
        ...
        edge_triangles = (
            bounding_box._edge_triangles + start_mesh_index + mesh_vertices_offset
        )
...
class BoundingBoxList:
    ...
    _vertices : np.ndarray
        MxN array of all displayed vertices from all bounding boxes where N is equal to ndisplay
    _index : np.ndarray
    ...
    def __init__(
        self, data: typing.Iterable[BoundingBox] = (), ndisplay: int = 2
    ) -> None:
    ...
    def _vertices_slice(self, bbox_index: int | np.integer) -> slice:
        ...
        start = self._vertices_index[bbox_index]
        bbox = self.bounding_boxes[bbox_index]
        return slice(start, start + bbox.data_displayed.shape[0])

    def _vertices_slice_available(self, bbox_index: int) -> slice:
        """Return the available slice of vertices for a given bounding box index."""
        start = self._vertices_index[bbox_index]
        end = self._vertices_index[bbox_index + 1]
        return slice(start, end)

    def _mesh_vertices_slice(self, bbox_index: int) -> slice:
        """Return the slice of mesh vertices for a given bounding box index."""
        start = self._mesh.vertices_index[bbox_index]
        bbox = self.bounding_boxes[bbox_index]
        return slice(start, start + bbox.vertices_count)

    def _mesh_vertices_slice_available(self, bbox_index: int) -> slice:
        """Return the available slice of mesh vertices for a given bounding box index."""
        start = self._mesh.vertices_index[bbox_index]
        end = self._mesh.vertices_index[bbox_index + 1]
        return slice(start, end)

    def _mesh_triangles_slice(self, bbox_index: int | np.integer) -> slice:
        """Return the slice of mesh triangles for a given bounding box index."""
        start = self._mesh.triangles_index[bbox_index]
        bbox = self.bounding_boxes[bbox_index]
        return slice(start, start + bbox.triangles_count)
    ...
    def _mesh_triangles_range_seq(
        self, bbox_indexes: IndexArray
    ) -> np.ndarray | slice:
        """Return the range of mesh triangles for a sequence of bounding box indexes."""
        if (
            bbox_indexes[-1] - bbox_indexes[0] == len(bbox_indexes) - 1
        ):  # If the sequence is continuous, return a range
            start = self._mesh.triangles_index[bbox_indexes[0]]
            end = self._mesh.triangles_index[bbox_indexes[-1]]
            end_bbox = self.bounding_boxes[bbox_indexes[-1]]
            return slice(start, end + end_bbox.triangles_count)
        # If the sequence is not continuous, return a numpy array
        ranges = [self._mesh_triangles_slice(i) for i in bbox_indexes]
        return self._slicess_to_array(ranges)

    def _vertices_range_seq(
        self, bbox_indexes: IndexArray
    ) -> np.ndarray | slice:
        """Return the range of vertices for a sequence of bounding box indexes."""
        if (
            bbox_indexes[-1] - bbox_indexes[0] == len(bbox_indexes) - 1
        ):  # If the sequence is continuous, return a range
            start = self._vertices_index[bbox_indexes[0]]
            end = self._vertices_index[bbox_indexes[-1]]
            end_bbox = self.bounding_boxes[bbox_indexes[-1]]
            return slice(start, end + end_bbox.data_displayed.shape[0])
        # If the sequence is not continuous, return a numpy array
        slicess = [self._vertices_slice(i) for i in bbox_indexes]
        return self._slicess_to_array(slicess)

    def _mesh_triangles_slice_available(self, bbox_index: int) -> slice:
        """Return the available slice of mesh triangles for a given bounding box index."""
        start = self._mesh.triangles_index[bbox_index]
        end = self._mesh.triangles_index[bbox_index + 1]
        return slice(start, end)

    def _mesh_vertices_face_slice(self, bbox_index: int) -> slice:
        """Return the slice of mesh vertices of face triangles for a given bounding box index."""
        bbox = self.bounding_boxes[bbox_index]
        start = self._mesh.vertices_index[bbox_index]
        return slice(start, start + bbox.face_vertices_count)

    def _mesh_triangles_face_slice(self, bbox_index: int) -> slice:
        """Return the slice of mesh triangles of face triangles for a given bounding box index."""
        bbox = self.bounding_boxes[bbox_index]
        start = self._mesh.triangles_index[bbox_index]
        return slice(start, start + bbox.face_triangles_count)

    def _mesh_vertices_edge_slice(self, bbox_index: int) -> slice:
        """Return the slice of mesh vertices of edge triangles for a given bounding box index."""
        bbox = self.bounding_boxes[bbox_index]
        start = (
            self._mesh.vertices_index[bbox_index] + bbox.face_vertices_count
        )
        return slice(start, start + bbox.edge_vertices_count)

    def _mesh_triangles_edge_slice(self, bbox_index: int) -> slice:
        """Return the slice of mesh triangles of edge triangles for a given bounding box index."""
        bbox = self.bounding_boxes[bbox_index]
        start = (
            self._mesh.triangles_index[bbox_index]
            + bbox.face_triangles_count
        )
    ...
    def _set_color(
        ...
        if not np.array_equal(colors.shape, (n_bounding_boxes, 4)):
            raise ValueError(
                trans._(
    ...
    @property
    def slice_key(self):
        """list: slice key for slicing n-dimensional bounding boxs."""
    ...
    def _update_displayed_triangles_to_bounding_box_index( # [NOTE] new addition changed from shape → bounding box
        ...
        for i in displayed_indices:
            ...
            elem_num = end - begin
            self._mesh.displayed_triangles_to_bounding_box_index[
    ...
    def _update_displayed_vertices_to_bounding_box_num( # [NOTE] new addition changed from shape → bounding box
        ...
        if (
            self.displayed_vertices_to_bounding_box_num.shape[0]
            != self.displayed_vertices.shape[0]
        ):
            self.displayed_vertices_to_bounding_box_num = np.full(
        ...
        for i in displayed_indices:
            ...
            elem_num = end - begin
            self.displayed_vertices_to_bounding_box_num[
                shift_idx : shift_idx + elem_num
            ] = i
            shift_idx += elem_num
    ...
    def _update_displayed(self) -> None: # [NOTE] changes require batch context
        ...
        self._update_displayed_triangles_to_bounding_box_index(disp_indices)
        ...
        self._update_displayed_vertices_to_bounding_box_num(disp_indices)
        self.displayed_index = self._vertices_index[disp_indices]

    def add(
        self,
        bounding_box: BoundingBox | Sequence[BoundingBox],
        face_color=None,
        edge_color=None,
        bounding_box_index=None,
        z_refresh=True,
    ):
        """Adds a single BoundingBox object (single add mode) or multiple BoundingBox (multiple bounding box mode, which is much faster)

        If bounding_box is a single instance of subclass BoundingBox then single add mode will be used, otherwise multiple add mode

        Parameters
        ----------
        bounding_box : single BoundingBox or iterable of BoundingBox
        face_color : color (or iterable of colors of same length as bounding_box)
        edge_color : color (or iterable of colors of same length as bounding_box)
        bounding_box_index : None | int
            ...
            Must be None in multiple bounding box mode.
        z_refresh : bool
        ...
        # multiple bounding box mode
            ...
            if bounding_box_index is not None:
                raise ValueError(
                    trans._(
                        'bounding_box_index_index must be None when adding multiple bounding boxes',
                        deferred=True,
                    )
                )
    ...
    def _add_single_bounding_box(
        ...
        if not issubclass(type(bounding_box), BoundingBox):
            raise TypeError(
                trans._(
                    'bounding box must be subclass of BoundingBox',
                    deferred=True,
                )
            )
        ...
        if bounding_box_index is None:
            ...
            z_refresh = False
            self.bounding_box[bounding_box_index] = bounding_box
            self._z_index[bounding_box] = bounding_box.z_index
        ...
        self._mesh.vertices_index = np.append(
            ...
            [len(self._mesh.vertices) + bounding_box.vertices_count],
            axis=0,
        )
        ...
        self._mesh.triangles_index = np.append(
            ...
            [len(self._mesh.triangles) + bounding_box.triangles_count],
            axis=0,
        )
    ...
    def _add_multiple_bounding_boxes(
        ...
        """Add multiple bounding_boxes at once (faster than adding them one by one)
        ...
        z_refresh : bool
            If set to true, the mesh elements are reindexed with the new z order.
            When bounding_boxes_index is provided, z_refresh will be overwritten to false,
            as the z indices will not change.
            When adding a batch of bounding boxes, set to false  and then call
            BoundingBoxList._update_z_order() once at the end.

        TODO: Currently shares a lot of code with `add()`, with the
        difference being that `add()` supports inserting bounding_box at a specific
        ...
        sizes = _calculate_array_sizes(bounding_boxes)

        # Preallocate arrays
        arrays = _preallocate_arrays(bounding_boxes, sizes)
    ...
    def update(self, index: int) -> None:
        """update bounding box at index `index`"""
    ...
    def _update_vertices(self, index: int) -> None:
        ...
        if bounding_box.data_displayed.shape[0] == curr_vert_count:
            # If the number of vertices is the same, just update the data
            self._vertices[vertices_slice] = bounding_box.data_displayed
        elif bounding_box.data_displayed.shape[0] < curr_vert_count:
            # To avoid relocation, we add first point few times for padding.
            new_slice = slice(
                vertices_slice.start,
                vertices_slice.start + bounding_box.data_displayed.shape[0],
            )
            padding_slice = slice(new_slice.stop, vertices_slice.stop)
            self._vertices[new_slice] = bounding_box.data_displayed
            self._vertices[padding_slice] = bounding_box.data_displayed[0]
        else:
            # there are more vertices in the bounding box than in the mesh
            # [NOTE] not sure that applies to bounding box except maybe 2D → 3D
            ...
            self._vertices = np.concatenate(
                [before_array, bounding_box.data_displayed, after_array]
            )
            self._vertices_index[index + 1 :] += (
                bounding_box.data_displayed.shape[0] - curr_vert_count
            )

    def _update_mesh_triangles(self, index: int) -> None:
        """Updates the mesh triangle data for a single bounding box located at index.
        ...
        new_triangle_count = bounding_box.triangles_count
        prev_vertices_slice = self._mesh_vertices_slice_available(index)
        prev_vertices_count = (
            prev_vertices_slice.stop - prev_vertices_slice.start
        )
        new_vertices_count = bounding_box.vertices_count
        if new_triangle_count <= current_triangles_count:
            face_slice = slice(
                triangles_slice.start,
                triangles_slice.start + bounding_box.face_triangles_count,
            )
            edge_slice = slice(
                triangles_slice.start + bounding_box.face_triangles_count,
                triangles_slice.start + bounding_box.triangles_count,
            )
            self._mesh.triangles[face_slice] = (
                bounding_box._face_triangles + triangle_shift
            )
            self._mesh.triangles_colors[face_slice] = self._face_color[index]
            self._mesh.triangles[edge_slice] = bounding_box._edge_triangles + (
                triangle_shift + bounding_box.face_vertices_count
            )
            ...
            # there are more triangles in the bounding box than in the mesh
            ...
            self._mesh.triangles_colors = np.concatenate(
                [
                    ...
                    np.repeat(
                        ...
                        bounding_box.face_triangles_count,
                        axis=0,
                    ),
                    np.repeat(
                        [self._edge_color[index]],
                        bounding_box.edge_triangles_count,
                        axis=0,
                    ),
    ...
    def remove(self, index: int, renumber: bool = True) -> None:
        ...
        renumber : bool
            ...
            list using `add_bounding_box`.
        """
        self.remove_multiple([index], renumber=renumber)

    @_batch_dec
    def remove_multiple(self, indices: list[int], renumber: bool = True):
        """Removes multiple bounding boxes located at indices.

        Parameters
        ----------
        indices : list of int
            Locations in list of the bounding boxes to be removed. Assumed to be sorted descending.
        renumber : bool
            Bool to indicate whether to renumber all bounding boxes or not.
        """
        if not indices:
            return
    ...
    def _update_mesh_vertices(self, index, edge=False, face=False):
        ...
        bounding_box = self.bounding_boxes[index]
        if edge and face:
            bbox_slice = self._mesh_vertices_slice_available(index)
            current_range = bbox_slice.stop - bbox_slice.start
            if current_range < bounding_box.vertices_count:
                # account for edge width
                edge_vertices_with_width = (
                    bounding_box._edge_vertices
                    + bounding_box.edge_width * bounding_box._edge_offsets
                )
                # need to allocate_more space
                self._mesh.vertices = np.concatenate(
                    [
                        self._mesh.vertices[: bbox_slice.start],
                        bounding_box._face_vertices,
                        edge_vertices_with_width,
                        self._mesh.vertices[bbox_slice.stop :],
                    ]
                )
                self._mesh.vertices_centers = np.concatenate(
                    [
                        self._mesh.vertices_centers[: bbox_slice.start],
                        bounding_box._face_vertices,
                        bounding_box._edge_vertices,
                        self._mesh.vertices_centers[bbox_slice.stop :],
                    ]
                )
                self._mesh.vertices_offsets = np.concatenate(
                    [
                        self._mesh.vertices_offsets[: bbox_slice.start],
                        np.zeros(
                            bounding_box._face_vertices.shape,
                            dtype=bounding_box._face_vertices.dtype,
                        ),
                        bounding_box._edge_offsets,
                        self._mesh.vertices_offsets[bbox_slice.stop :],
                    ]
                )
                diff = bounding_box.vertices_count - current_range
                self._mesh.vertices_index[index + 1 :] += diff
                return
            if current_range > bounding_box.vertices_count:
                zeros_slice = slice(
                    bbox_slice.start + bounding_box.vertices_count, bbox_slice.stop
                )
    ...
    def update_edge_color(
        ...
        index : int
            Location in list of the bounding box to be changed.
        edge_color : np.ndarray of bounding box (4,)
    ...
    def update_face_color(
        ...
        index : int
            Location in list of the bounding box to be changed.
        face_color : np.ndarray of bounding box (4,)
    ...
    def outline(
        ...
        indices : int | Sequence[int]
            Location in list of the bounding boxes to be outline.
        ...
        if not isinstance(indices, Sequence):
            bounding_box = self.bounding_boxes[indices]
            return (
                bounding_box._edge_vertices,
                bounding_box._edge_offsets,
                bounding_box._edge_triangles,
    ...
    def outlines(
        ...
        """Finds outlines of bounding boxes listed in indices, using chunked processing.
        ...
        # of performance for a wide range of bounding box counts.
        chunk_size = 500
        ...
        for start in range(0, len(indices), chunk_size):
            ...
            chunk_bounding_boxes = [self.bounding_boxes[i] for i in chunk_indices]
            ...
            for bb in chunk_bounding_boxes:
                ...
                chunk_offsets.append(bb._edge_offsets)
                tris = bb._edge_triangles
                chunk_tris.append(tris)
                n_verts_per_bbox.append(verts.shape[0])
                n_tris_per_bbox.append(tris.shape[0])
            ...
            if triangles.size > 0:
                vert_offsets = np.zeros(
                    len(n_verts_per_bbox), dtype=triangles.dtype
                )
                if len(vert_offsets) > 1:
                    np.cumsum(n_verts_per_bbox[:-1], out=vert_offsets[1:])
                tri_offsets = np.repeat(vert_offsets, n_tris_per_bbox)
                triangles = (
    ...
    def bounding_boxes_in_box(
        ...
        Returns
        -------
        bounding_boxes : list of ints
            List of bounding boxes that are inside the box.
        """
        bbox_mins, bbox_maxs = self._bounding_boxes
        if bbox_mins.shape[0] == 0:
            return []

        selection_min = np.min(corners, axis=0)
        selection_max = np.max(corners, axis=0)

        # If the box encompasses all bounding boxes, just get them directly
        layer_min = np.min(bbox_mins, axis=0)
        layer_max = np.max(bbox_maxs, axis=0)
        if np.all(selection_min <= layer_min) and np.all(
            selection_max >= layer_max
        ):
            return self._visible_bounding_boxes_indices.tolist()

        # Get bounding boxes with (napari) bounding boxes intersecting the selection box
        intersects_mask = np.all(bbox_maxs >= selection_min, axis=1) & np.all(
            bbox_mins <= selection_max, axis=1
        )

        intersecting_indices = self._visible_bounding_boxes_indices[intersects_mask]

        if intersecting_indices.size == 0:
            return []

        bbox_mins = bbox_mins[intersects_mask]
        bbox_maxs = bbox_maxs[intersects_mask]

        bbox_full_in_mask = np.all(
            bbox_maxs <= selection_max, axis=1
        ) & np.all(bbox_mins >= selection_min, axis=1)

        return [
            num
            for num, full_in in zip(
                intersecting_indices, bbox_full_in_mask, strict=True
            )
            if full_in
    ...
    def _visible_bounding_boxes(self) -> list[tuple[int, BoundingBox]]:
        ...
        if len(slice_key):
            return [
                ...
                if (
                    np.all(bb.slice_key[0] <= slice_key)
                    and np.all(slice_key <= bb.slice_key[1])
                )
            ]
    ...
    def _bounding_boxes(
        ...
        data = np.array([bb[1].bounding_box for bb in self._visible_bounding_boxes])
        if data.size == 0:
            return np.empty((0, self.ndisplay)), np.empty((0, self.ndisplay))  # type: ignore[return-value]
        return data[:, 0], data[:, 1]

    @cached_property
    def _visible_bounding_boxes_indices(
        self,
    ) -> np.ndarray[tuple[int], np.dtype[IndexDtype]]:
        return np.array([bb[0] for bb in self._visible_bounding_boxes])

    # [NOTE] here some variables are called bounding_boxes to refer to napari defined internal bounding boxes
    # may be good to make that distinction clear
    def inside(self, coord):
        ...
        if not self.bounding_boxes:
            return None
        nap_bounding_boxes = self._bounding_boxes
        in_bbox = np.all(
            (nap_bounding_boxes[0] <= coord) * (nap_bounding_boxes[1] >= coord),
            axis=1,
        )
        ...
        try:
            z_index = [
                self._visible_bounding_boxes[i][1].z_index for i in inside_indices
            ]
            pos = np.argsort(z_index)
            return self._visible_bounding_boxes[
                next(
                    inside_indices[p]
                    for p in pos[::-1]
                    if np.any(
                        inside_triangles(
                            self._visible_bounding_boxes[inside_indices[p]][
                                1
                            ]._all_triangles()
                            - coord
                        )
    ...
    def to_masks(self, mask_shape=None, zoom_factor=1, offset=(0, 0)):
        ...
        masks = np.array(
            [
                bb.to_mask(mask_shape, zoom_factor=zoom_factor, offset=offset)
    ...
    def to_labels(self, labels_shape=None, zoom_factor=1, offset=(0, 0)):
        """Returns a integer labels image, where each bouunding box is embedded in an
    ...
    def to_colors(
        ...
        bbox_in_view = np.argwhere(self._displayed)
        z_order_in_view_mask = np.isin(z_order, bbox_in_view)
        ...
        if max_bounding_boxes is not None and len(z_order_in_view) > max_bounding_boxes:
            z_order_in_view = z_order_in_view[-max_bounding_boxes:]
    ...
    def _clear_cache(self):
        ...
        self.__dict__.pop('_visible_bounding_boxes', None)
        self.__dict__.pop('_visible_bounding_boxes_indices', None)