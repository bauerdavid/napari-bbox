...
class BoundingBoxLayer(Layer):
    ...
    data : list or array
        ...
        array if each Bounding box has the same number of vertices.
    ndim : int
    ...
    _selected_data_stored : set
        ...
        same highlighted bounding box when no data has changed.
    _selected_box : None | np.ndarray
        `None` if no bounding box are selected, otherwise a 10x2 array of vertices of
    ...
    _drag_box : None | np.ndarray
        If a drag box is being created to select bounding box then this is a 2x2
    ...
    _is_moving : bool
        Bool indicating if any bounding box are currently being moved.
    _is_selecting : bool
        Bool indicating if a drag box is currently being created in order to
        select bounding box.
    _is_creating : bool
        Bool indicating if any bounding box are currently being created.
    _fixed_aspect : bool
        Bool indicating if aspect ratio of bounding box should be preserved on
        resizing.
    _aspect_ratio : float
    ...
    _update_properties : bool
        ...
        bounding box when they are changed. Blocking this prevents circular loops
        when bounding box are selected and the properties are changed based on that
        selection
    ...
    _vertex_size : float
        Size of the vertices of the bounding box and bounding box in Canvas
        coordinates.
    ...
    _thumbnail_update_thresh : int
        If there are more than this number of bounding box, the thumbnail
    ...
    _drag_modes: ClassVar[dict[Mode, Callable[[BoundingBoxLayer, Event], Any]]] = {
        Mode.PAN_ZOOM: no_op,
    ...
    _move_modes: ClassVar[dict[Mode, Callable[[BoundingBoxLayer, Event], Any]]] = {
        Mode.PAN_ZOOM: no_op,
    ...
    _double_click_modes: ClassVar[
        dict[Mode, Callable[[BoundingBoxLayer, Event], Any]]
    ] = {
        Mode.PAN_ZOOM: no_op,
    ...
    def data(self, data):
        ...
        self._add_bounding_boxes(
            ...
            face_color=face_color,
            z_index=z_indices,
            n_new_shapes=n_new_bounding_boxes,
        )
        self._update_dims()
    ...
    def selected_data(self) -> Selection[int]:
        """Selection: set of currently selected bounding boxes."""
    ...
    def add_bounding_boxes(
        ...
        *,
        ...
        data : Array | List[Array]
            List of bounding box data where each element is a (4, D) array of 4 vertices
    ...
    def add(
        ...
        *,
        ...
        z_index : int | list
            ...
            bounding boxes.
        gui : bool
            Whether the bounding box is drawn by drawing in the gui.
        """
        data = np.asarray(data)
        ...
        if n_new_bounding_boxes > 0:
            ...
            self._add_bounding_boxes(
                ...
                face_color=face_color,
                z_index=z_index,
                n_new_bounding_boxes=n_new_bounding_boxes,
            )
        ...
        *,
        ...
        with self.block_thumbnail_update():
            self._add_bounding_boxes(
                ...
                face_color=face_color,
                z_index=z_index,
                n_new_bounding_boxes=n_bounding_boxes, 
            )
    ...
    def _add_bounding_boxes(
        ...
        *,
        ...
        z_index : int | list
            ...
            applied to each bounding box otherwise the same value will be used for all
            bounding boxes.
        n_new_bounding_boxes : int
            The number of new bounding boxes to be added to the BoundingBox layer.
        """
        if n_new_bounding_boxes > 0:
    ...
    def _add_bounding_boxes_to_view(self, bounding_box_inputs, data_view: BoundingBoxList):
        """Build new bounding_boxes and add them to the _data_view"""
        bounding_box_inputs = tuple(bounding_box_inputs)

        # build all bounding boxes
        bounding_box_inp = tuple(
            (
                BoundingBox(
                    d,
                    edge_width=ew,
        ...
        bounding_boxes, edge_colors, face_colors = tuple(zip(*bounding_box_inp, strict=False))

        # Add all bounding boxes at once (faster than adding them one by one)
        data_view.add(
    ...
    def interaction_box(self, index: int | Iterable[int]) -> BoxArray | None:
        ...
        index : int or iterable of int
            Index of a single bounding box, list of bounding boxes or Selection around which to
        ...
        if isinstance(index, Iterable):
            ...
            if len(index) == 0:
                ...
                box = copy(self._data_view.bounding_boxes[next(iter(index))]._box)
            else:
                displayed_bounding_box_indices = [
                    i for i in index if self._data_view._displayed[i]
                ]
                if not displayed_bounding_box_indices:
                    box = None
                else:
                    mask = np.isin(
    ...
    def _update_thumbnail(self, event=None):
        ...
        colormapped[..., 3] = 1
        # if the bounding box layer is empty, don't update, just leave it black
        if len(self.data) == 0:
    ...
    def remove(self, indices: list[int]) -> None:
        ...
        ----------
        indices : List[int]
            List of indices of bounding boxes to remove from the layer.
        """
    ...
    def get_bounding_box_info(self, index: int) -> dict[str, Any]:
        ...
        -------
        info : dict
            A dictionary containing all relevant details of the bounding box.
        """
    ...
    def pop(self, index=-1) -> dict[str, Any]:
        """Remove and return the bounding box at the given index.

        Parameters
        ----------
        index : int
            Index of the bounding box to remove and return. Defaults to -1, which
            removes the last bounding box.

        Returns
        -------
        dict[str, Any]
            Dictionary containing the data for the removed bounding box.
        """
        if index == -1:
    ...
    def _scale_box(self, scale, center=(0, 0)):
        ...
        #if not np.array_equal(box[Box.TOP_CENTER], box[Box.HANDLE]): # [NOTE] edited out
    ...
    def _get_index_and_intersection(
        ...
        """Get the bounding box index and intersection point of the first bounding box
        ...
        intersection_point : Union[None, np.ndarray]
            (n,) array containing the point where the ray intersects the first bounding box
            (i.e., the bounding box most in the foreground). The coordinate is in layer
            coordinates.
        """
    ...
    def get_index_and_intersection(
        ...
        if (start_point is not None) and (end_point is not None):
            bounding_box_index, intersection_point = self._get_index_and_intersection(
                start_point=start_point,
                end_point=end_point,
                dims_displayed=dims_displayed,
            )
        ...
        return bounding_box_index, intersection_point
    ...
    def _paste_data(self) -> None:
        """Paste any bounding boxes from clipboard and then selects them."""
